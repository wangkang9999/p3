...
public class Play extends Activity {
    ...
    protected void onCreate (Bundle x){
        ...
        for (int i=0;i<9;i++){
            for (int j=0;j<9;j++){
                ...
                if (t2.mxt[i][j]==0){
                    ...
                    mx.get(i).get(j).setTextColor(this.getResources().getColor(R.color.colorblue));
                    ...
                    mx.get(i).get(j).setFocusable(false);
                }
                if ((i/3*3+j/3)%2==0 ){
        ...
        TextView tlvl=(TextView)findViewById(R.id.textView4);
        String slvl="Level: "+ String.valueOf(9-t1.lvl);
        tlvl.setText(slvl);
            ...
            public void onClick(View view) {
                ...
                for  (int i=0;i<9;i++){
                    for (int j=0;j<9;j++){
                        ...
                        if (s.equals("")){
                            mx.get(i).get(j).setText(String.valueOf(t3.mxt[i][j]));
                            mx.get(i).get(j).setTextColor(0xff5bb75b);
                        }else if (Integer.valueOf(s)==t3.mxt[i][j]){

                        }else {
                            mx.get(i).get(j).setText(String.valueOf(t3.mxt[i][j]));
                            mx.get(i).get(j).setTextColor(0xffda4f49);
                        }
                    }
                }
            }
        ...
        btn11.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                ...
                if (!t4.checkfinish()) {
                    ...
                    output="你的答案在 ";
                    k=t4.index;
                    if (t4.workon.equals("row")){
                        temp=t4.getRow(k);
                        output+="行：";
                    }else if(t4.workon.equals("col")){
                        temp=t4.getCol(k);
                        output+="列：";
                    }else{
                        temp=t4.getgroup(k);
                    ...
                    output+=String.valueOf(t4.index+1)+" 有错误.";
                    d2=new AlertDialog.Builder(Play.this).setTitle("ERROR").setMessage(output).setPositiveButton("OK",null).show();
                }
            }
        });
